...
module.exports = function(app, passport) {
    ...
    app.param('pid', function(req, res, next, id) {
        //validate project id
        console.log("[ROUTE.param] ---------------------------------");
        console.log("[ROUTE.param] validating project id.");
        db.proj.findOne({
            _id: req.params.pid
        }, function(err, docs) {
            if (!docs || !docs._id) {
    ...
    app.param('stid', function(req, res, next, id) {
        //validate project id
        console.log("[ROUTE.param] ---------------------------------");
        console.log("[ROUTE.param] validating survey template id.");
        db_old.surveyTemplate().findOne({
            _id: req.params.stid
        }, function(err, docs) {
    ...
    app.param('uid', function(req, res, next, id) {
        //validate user id
        console.log("[ROUTE.param] --------------------------------");
        console.log("[ROUTE.param] Validating user id");
        db.user.findOne({
            _id: req.params.uid
        }, function(err, docs) {
            if (!docs || docs.length === 0) {
                console.log('[ROUTE.param] User [id: ' + req.params.uid + '] not found!');
                //next();
                res.send(404, "Error: User not found");
            } else {
                req.user_q = docs;
                console.log('[ROUTE.param] User [id: ' + req.user_q._id + '] found!');
                next();
            }
        });
    });
    ...
    app.param('plid', function(req, res, next, id) {
        ...
        db.player.findOne({
            ...
            if (!docs || docs.length === 0) {
                ...
                //next();
                res.send(404, "Error: Player not found");
            } else {
                req.player = docs;
    ...
    app.param('oid', function(req, res, next, id) {
        //validate org id
        console.log("[ROUTE.param] --------------------------------");
        console.log("[ROUTE.param] Validating org id");
        orgModel.listById(req.params.oid, function(err, docs) {
            if (!docs || docs.length === 0) {
                console.log('[ROUTE.param] org [id: ' + req.params.oid + '] not found!');
                res.send(404, "Error: org not found");
            } else {
                req.org = docs;
