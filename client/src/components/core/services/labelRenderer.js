angular.module('common')
    .service('labelRenderer', ['$q', 'renderGraphfactory', 'graphHoverService', 'nodeSelectionService', 'layoutService', 'FilterPanelService', 'SelectorService',
        function($q, renderGraphfactory, graphHoverService, nodeSelectionService, layoutService, FilterPanelService, SelectorService) {
    ...
    **************************************/
            ...
            function labelClick(d, groupAttrInfo) {
                _clearTimer();

                d.isSelected = true;

                // var filterConfig = FilterPanelService.getFilterForId(groupAttrInfo.attr.id);

                // graphHoverService.clearHovers(true);

                // renderGraphfactory.sig().dispatchEvent('clickNodes', {
                //     node: d.nodes,
                //     degree: 0,
                //     all: true,
                //     genBy: 'labelClick'
                ...
                // renderGraphfactory.sig().dispatchEvent('overNodes', {
                //     node: d.nodes,
                //     degree: 0,
                //     all: true,
                //     genBy: 'labelClick'
                ...
                // nodeSelectionService.addNodesToSelected(d.nodes);
                // nodeSelectionService.highlightAllSelected(true);
                selectFilter(groupAttrInfo.attr.id, d.id);
                nodeSelectionService.selectionActionByAttributes(groupAttrInfo.attr.id, getSelectedValues(groupAttrInfo.attr.id));
            }

            function selectFilter (attrId, value) {
                var filterConfig = FilterPanelService.getFilterForId(attrId);

                filterConfig.isEnabled = true;
                filterConfig.state.selectedVals = filterConfig.state.selectedVals || [];
                filterConfig.state.selectedVals = filterConfig.state.selectedVals.concat([value]);
                filterConfig.selector = filterConfig.isEnabled ? genSelector(filterConfig.state.selectedVals, attrId) : null;
            }

            function genSelector (selectedVals, attrId) {
                var selector = SelectorService.newSelector();
                selector.ofMultipleAttrValues(attrId, selectedVals, true);
                return selector;
            }

            function getSelectedValues(attrId) {
                var filterConfig = FilterPanelService.getFilterForId(attrId);
            ...
            function labelHover(d) {
                var hoverFn = function() {
                    ...
                    // hoverTimer = undefined;
                    nodeSelectionService.addNodesToTempHighlightedNodes(hoverData.nodes);
                    nodeSelectionService.highlightAllSelected(true);
                };
                _clearTimer();
                hoverData = d;
            ...
            function labelUnHover(d) {
                ...
                //     graphHoverService.clearHovers(true);
                // }
                nodeSelectionService.removeNodesFromTempHighlightedNodes(d.nodes);
                nodeSelectionService.highlightAllSelected(true);
            ...
            function d3NodeLabelDefRender (node, canvas, settings) {
                ...
                if(node.isGroup) {
                    canvas.on("click", toShow ? function labelClickHandler(d) {
                        return labelClick(d, groupInfo);
                    } : null)
