...
function($rootScope, $q, renderGraphfactory, dataGraph, nodeRenderer, inputMgmtService, BROADCAST_MESSAGES) {
    ...
    **************************************/
    ...
    function draw(renderLabel) {
        ...
        // an optimization for tweening
        if(isTweening) {
            if(_.isEmpty(selectedNodeNeighbours))
                return;
        }
        ...
        var graph = renderGraphfactory.sig().graph,
            ...
            d3sel = renderer.d3Sel.selections(),
            nodes = _.values(selectedNodeNeighbours);

        var getNode = graph.nodes,
            embedSettings = settings.embedObjects({
                prefix: renderer.options.prefix,
                inSelMode: !_.isEmpty(selectedNodes)
            });

        var nodeId = window.mappr.utils.nodeId;

        contexts.selections.canvas.width = contexts.selections.canvas.width;

        renderer.greyout(!_.isEmpty(selectedNodeNeighbours), nodesInPop(nodes) ? 'pop' : 'select');

        // Render
        if (embedSettings('enableSelections')) {
            ...
            _.each(edges, function(o) {
                (sigma.canvas.edges[o.type] || sigma.canvas.edges.def)(
                    o,
                    selectedNodeNeighbours[o.source] || getNode(o.source),
                    selectedNodeNeighbours[o.target] || getNode(o.target),
                    contexts.selections,
                    embedSettings,
            ...
            // ctx = contexts.selections;
            // // Render the red circle for the node on the canvas
            // _.each(nodes, function(n) {
            //  (sigma.canvas.nodes[n.type] || sigma.canvas.nodes.def)(
            //      n,
            //      contexts.selections,
            //      embedSettings,
            //      renderer.displayScale
            ...
            if(settings('tweening')) {
                    ...
                    .append('div')
                        ...
                        .each(function hnc (node) {
                            if(node.state === 'highlight')
                                nodeRenderer.d3NodeHighlightCreate(node, d3.select(this), embedSettings);
                            else if(node.state === 'selected')
                                nodeRenderer.d3NodeSelectedCreate(node, d3.select(this), embedSettings);
                            else if(node.state === 'pop')
                                nodeRenderer.d3NodePopCreate(node, d3.select(this), embedSettings);
                            else
                                nodeRenderer.d3NodeHighlightCreate(node, d3.select(this), embedSettings);
                        });
                // sort nodes by size and by select state
                ...
                mainSel
                    .each(function hnr (node) {
                        if(node.state === 'highlight')
                            nodeRenderer.d3NodeHighlightRender(node, d3.select(this), embedSettings);
                        else if(node.state === 'selected')
                            nodeRenderer.d3NodeSelectedRender(node, d3.select(this), embedSettings);
                        else if(node.state === 'pop')
                            nodeRenderer.d3NodePopRender(node, d3.select(this), embedSettings);
                        else
                            nodeRenderer.d3NodeHighlightRender(node, d3.select(this), embedSettings);
                    });
            }

            // Render node labels in selection state unless tweening.
            if(renderLabel && !isTweening) {
                // defined in labelService.
                sigma.d3.labels.sel(
                    _.reject(nodes, 'isAggregation'),
                    renderer.d3Sel.labels(),
                    embedSettings
                );
            }
        }
